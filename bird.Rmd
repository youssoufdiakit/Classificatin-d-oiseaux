...
les boites à moustache , nous montre plusieurs valeurs aberrantes pour chaque variable en fonction du type d'oiseaux. Les valeurs manquantes se situent seulement sur 7 observations , nous décidons de les supprimer. Donc au lieu de 420 observations , nous allons faire notre etudes sur une base de 413 observation , ce qui est largement suffisant pour notre études
...
coords <- acp$ind$coord[, 1:2]  # On prend les axes 1 à 4
...
```{r}
# Application de la méthode d'agglomération (Ward.D2)
CAH.ward <- hclust(dist_matrix, method = "ward.D")

# Affichage du dendrogramme
plot(CAH.ward , main = "Dendogramme avec la methode de Ward" , hang=-1)

rect.hclust(CAH.ward, 6, border ="green")
sil_ward <- silhouette(cutree(CAH.ward, k = 2), dist(coords))
mean(sil_ward[, 3])  # Moyenne des largeurs
...
plot(rev(CAH.max$height),type="b")
...
sil_max <- silhouette(cutree(CAH.max, k = 6), dist(coords))
mean(sil_max[, 3])
...
# 2. Adjusted Rand Index (nécessite mclust)
library(mclust)
adjustedRandIndex(type_oiseau, clusters_max)
...
sil_ward <- silhouette(clusters_max, dist(coords))
mean(sil_ward[, 3])  # Moyenne des largeurs

sil_max <- silhouette(cutree(CAH.max, k = 2), dist(coords))
mean(sil_max[, 3])
...
plot(silhouette_result, main = "Méthode de Silhouette")
